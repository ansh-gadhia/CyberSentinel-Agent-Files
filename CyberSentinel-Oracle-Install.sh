...
# ============================================================
# CyberSentinel Installer Script for Oracle Linux (9 / 10+)
# ============================================================
...
# OS DETECTION
# ============================================================
step "OS Detection │ Identifying Oracle Linux Version"

if ! grep -qi "oracle" /etc/os-release 2>/dev/null && [ ! -f /etc/oracle-release ]; then
    error "This installer only supports Oracle Linux."
    exit 1
fi

OS_MAJOR=$(rpm -q --queryformat '%{VERSION}' oraclelinux-release 2>/dev/null \
    || grep -oP '(?<=VERSION_ID=")[0-9]+' /etc/os-release \
    || grep -oP '[0-9]+' /etc/oracle-release | head -1)
OS_MAJOR=$(echo "$OS_MAJOR" | grep -oP '^[0-9]+')

[ -z "$OS_MAJOR" ] && { error "Could not determine Oracle Linux version."; exit 1; }
[ "$OS_MAJOR" -lt 9 ] && { error "Oracle Linux ${OS_MAJOR} is not supported. Requires Oracle Linux 9+."; exit 1; }

success "Detected: Oracle Linux ${OS_MAJOR}"
...
echo -e "${CYAN}${BOLD}╚══════════════════════════════════════════════╝${NC}"
echo -e "  OS           : ${BOLD}Oracle Linux ${OS_MAJOR}${NC}"
