...
try {
    ...
    # ========================================
    # Check agent quota before registration
    # ========================================
    $SiemServer = $managerIP
    $QuotaPort  = "5100"
    Write-Host ""
    Write-Host "  Checking agent registration quota..." -ForegroundColor Cyan

    try {
        $quotaResponse = Invoke-RestMethod -Uri "http://${SiemServer}:${QuotaPort}/api/agents/can-register" -TimeoutSec 5
    } catch {
        Write-Host "  ✗ Cannot reach quota API at ${SiemServer}:${QuotaPort}" -ForegroundColor Red
        Write-Log "Quota API unreachable at ${SiemServer}:${QuotaPort} — $_" -Level "ERROR"
        Read-Host "Press Enter to exit"; exit 1
    }

    if (-not $quotaResponse.allowed) {
        Write-Host "  ✗ REJECTED: $($quotaResponse.reason)" -ForegroundColor Red
        Write-Log "Quota check rejected: $($quotaResponse.reason)" -Level "ERROR"
        Read-Host "Press Enter to exit"; exit 1
    }

    Write-Host "  ✓ $($quotaResponse.reason)" -ForegroundColor Green
    Write-Log "Quota check passed: $($quotaResponse.reason)" -Level "SUCCESS"
    Write-Host ""
