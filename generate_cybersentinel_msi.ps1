# Created by Virtual Galaxy Infotech Ltd.
# Based on Wazuh agent installer script
# This program is a free software; you can redistribute it and/or modify it under the terms of GPLv2

param (
    [string]$MSI_NAME = "cybersentinel-agent.msi",
    [string]$SIGN = "no",
...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows CyberSentinel agent msi package.

    PARAMETERS TO BUILD CYBERSENTINEL-AGENT MSI (OPTIONALS):
    ...
    USAGE:
        ...
        * CYBERSENTINEL:
          $ ./generate_cybersentinel_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}
            Build a devel msi:    $ ./generate_cybersentinel_msi.ps1 -MSI_NAME cybersentinel-agent_4.14.2-0_windows_dev.msi -SIGN no
            Build a prod msi:     $ ./generate_cybersentinel_msi.ps1 -MSI_NAME cybersentinel-agent-4.14.2-1.msi -SIGN yes
    "
    Exit
}
...
function BuildCyberSentinelMsi(){
    ...
    Write-Host "Building CyberSentinel MSI installer..."

    & $CANDLE_EXE -nologo .\cybersentinel-installer.wxs -out "cybersentinel-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\cybersentinel-installer.wixobj" -out $MSI_NAME -ext WixUtilExtension -ext WixUiExtension
...
function ExtractDebugSymbols(){
  ...
  #compress every pdb file in current folder
    ...
    $ZIP_NAME = $MSI_NAME -replace 'cybersentinel-agent', 'cybersentinel-agent-debug-symbols' -replace '\.msi$', '.zip'
