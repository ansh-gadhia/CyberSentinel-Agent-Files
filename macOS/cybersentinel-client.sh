...
# Author: Virtual Galaxy <SOCAsS@vgipl.in>
...
AUTHOR="Virtual Galaxy Infotech Ltd."
...
check_folders()
{
    ...
    if [ ! -d $ALERTS_FOLDER ]
    then
        if rm -rf $ALERTS_FOLDER && mkdir -p $ALERTS_FOLDER && chown cybersentinel:cybersentinel $ALERTS_FOLDER && chmod 770 $ALERTS_FOLDER
        then
...
start_service()
{
    echo "Starting CyberSentinel $VERSION..."
    checkpid;
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                echo "${pfile}: Process ${pid} not used by CyberSentinel, removing .."
...
stop_service()
{
    ...
    echo "CyberSentinel $VERSION Stopped"
}

info()
{
     if [ "X${1}" = "X" ]; then
        echo "CyberSentinel_VERSION=\"${VERSION}\""
        echo "CyberSentinel_REVISION=\"${REVISION}\""
        echo "CyberSentinel_TYPE=\"${TYPE}\""
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
            -r) echo "${REVISION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/cybersentinel-agentd//')
    restart_service
    # Signal agentd (SIGUSR1) to reload (reconnects execd)
    pid=`cat ${DIR}/var/run/cybersentinel-agentd-*.pid`
    kill -USR1 $pid
    ;;
status)
    lock
