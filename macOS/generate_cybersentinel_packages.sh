#!/bin/bash
set -e
# Program to build and package OSX cybersentinel-agent
# CyberSentinel package generator
...
SERVICE_PATH="/Library/LaunchDaemons/com.cybersentinel.agent.plist"
STARTUP_PATH="/Library/StartupItems/CYBERSENTINEL/StartupParameters.plist"
LAUNCHER_SCRIPT_PATH="/Library/StartupItems/CYBERSENTINEL/CyberSentinel-launcher"
STARTUP_SCRIPT_PATH="/Library/StartupItems/CYBERSENTINEL/CYBERSENTINEL"
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in CyberSentinel's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    packaged_directory=$CURRENT_PATH/cybersentinel-agent/payload

    if [ -d "$CURRENT_PATH/cybersentinel-agent" ]; then

        echo "\nThe cybersentinel agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/cybersentinel-agent
    fi

    munkipkg --create --json $CURRENT_PATH/cybersentinel-agent

    cp -f $build_info_file $CURRENT_PATH/cybersentinel-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/cybersentinel-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/cybersentinel-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/cybersentinel-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/cybersentinel-agent/scripts/postinstall
...
function build_package() {
    ...
    # Use local source or clone from repository
    if [ -n "$BRANCH_TAG" ]; then
        ...
        # Use a fixed hash tag since we are using local rebranded source
        short_commit_hash="custom"
    fi
    ...
    if [ $IS_STAGE == "no" ]; then
        pkg_name="cybersentinel-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="cybersentinel-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe cybersentinel agent is already installed on this machine."
        echo "Removing it from the system."
    ...
    # create package
    if munkipkg $CURRENT_PATH/cybersentinel-agent ; then
        echo "The cybersentinel agent package for macOS has been successfully built."
        mv $CURRENT_PATH/cybersentinel-agent/build/* $DESTINATION/
        symbols_pkg_name="cybersentinel-agent-debug-symbols-${VERSION}-${REVISION}.${ARCH}-macos"
...
function help() {
    ...
    echo "    -b, --branch <branch>         [Optional] Select Git branch. If omitted, uses local source."
...
function main() {
    ...
    if [ "${BUILD}" = "no" ] && [ "${NOTARIZE}" = "no" ]; then
        echo "No build branch specified and notarization not selected."
        help 1
    fi
